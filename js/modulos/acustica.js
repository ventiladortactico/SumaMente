// Objeto mejorado para manejar audio continuo y en tiempo real sin cortes
const AcusticaAudio = {
    audioContext: null,
    oscillator: null,
    gainNode: null,
    ...
    // Inicia un tono continuo (ideal para cuando el usuario empieza a mover un slider)
    ...
    // Modifica la frecuencia de manera fluida y sin "clicks" de audio
    updateFrequency: function(frequency) {
        if (this.isPlaying && this.oscillator) {
            // Evitamos frecuencias negativas o fuera del rango audible estándar
    ...
    // Modifica el volumen en tiempo real de forma segura
    updateVolume: function(volume) {
        if (this.isPlaying && this.gainNode) {
            const safeVolume = Math.min(Math.max(volume, 0), 0.5); // Límite de seguridad de 0.5
            this.gainNode.gain.linearRampToValueAtTime(safeVolume, this.audioContext.currentTime + 0.05);
        }
    },

    // Detiene el tono por completo liberando los nodos de la Web Audio API
    ...
    // Mantiene la reproducción clásica por ráfagas cortas si es necesario
